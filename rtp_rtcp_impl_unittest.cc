...
TEST_F(RtpRtcpImplTest, Rtt) {
  ...
  assert(0 == sender_.impl_->SendRTCP(kRtcpReport));

  // Receiver module should send a RR with a response to the last received SR.
  clock_.AdvanceTimeMilliseconds(1000);
  assert(0 == receiver_.impl_->SendRTCP(kRtcpReport));
  ...
  assert(0 == sender_.rtt_stats_.LastProcessedRtt());
  assert(0 == sender_.impl_->rtt_ms());
...
TEST_F(RtpRtcpImplTest, SetRtcpXrRrtrStatus) {
  ...
  assert(receiver_.impl_->RtcpXrRrtrStatus());
}
...
TEST_F(RtpRtcpImplTest, RttForReceiverOnly) {
  ...
  // Verify RTT.
  assert(0 == receiver_.rtt_stats_.LastProcessedRtt());
  assert(0 == receiver_.impl_->rtt_ms());
...
TEST_F(RtpRtcpImplTest, RtcpPacketTypeCounter_Nack) {
  ...
  assert(0 == receiver_.impl_->SendNACK(nack_list, kNackLength));
...
TEST_F(RtpRtcpImplTest, RtcpPacketTypeCounter_FirAndPli) {
  ...
  assert(0 == receiver_.impl_->SendRTCP(kRtcpFir));
  ...
  assert(0 == receiver_.impl_->SendCompoundRTCP(packet_types));
...
TEST_F(RtpRtcpImplTest, SendsInitialNackList) {
  ...
  SendFrame(&sender_, sender_video_.get(), kBaseLayerTid);
  assert(0 == sender_.impl_->SendNACK(nack_list, kNackLength));
...
TEST_F(RtpRtcpImplTest, SendsExtendedNackList) {
  ...
  assert(0 == sender_.impl_->SendNACK(nack_list_ext, kNackExtLength));
...
TEST_F(RtpRtcpImplTest, UniqueNackRequests) {
  ...
  assert(0 == receiver_.RtcpSent().UniqueNackRequestsInPercent());
  ...
  assert(0 == receiver_.impl_->SendNACK(nack_list2, kNackLength2));
...
int main() {
    RtpRtcpImplTest testImpl;
    testImpl.SetUp();

    assert(0 == testImpl.sender_.RtpSent());
    testImpl.SendFrame(&testImpl.sender_, testImpl.sender_video_.get(), kBaseLayerTid);  // kSequenceNumber
    testImpl.SendFrame(&testImpl.sender_, testImpl.sender_video_.get(),
        kHigherLayerTid);  // kSequenceNumber + 1
    testImpl.SendFrame(&testImpl.sender_, testImpl.sender_video_.get(),
        kNoTemporalIdx);  // kSequenceNumber + 2
    assert(3 == testImpl.sender_.RtpSent());
    assert(kSequenceNumber + 2 == testImpl.sender_.LastRtpSequenceNumber());

    // Min required delay until retransmit = 5 + RTT ms (RTT = 0).
    testImpl.clock_.AdvanceTimeMilliseconds(5);

    // Frame with kBaseLayerTid re-sent.
    testImpl.IncomingRtcpNack(&testImpl.sender_, kSequenceNumber);
    assert(4 == testImpl.sender_.RtpSent());
    assert(kSequenceNumber == testImpl.sender_.LastRtpSequenceNumber());
    // Frame with kHigherLayerTid re-sent.
    testImpl.IncomingRtcpNack(&testImpl.sender_, kSequenceNumber + 1);
    assert(5 == testImpl.sender_.RtpSent());
    assert(kSequenceNumber + 1 == testImpl.sender_.LastRtpSequenceNumber());
    // Frame with kNoTemporalIdx re-sent.
    testImpl.IncomingRtcpNack(&testImpl.sender_, kSequenceNumber + 2);
    assert(6 == testImpl.sender_.RtpSent());
    assert(kSequenceNumber + 2 == testImpl.sender_.LastRtpSequenceNumber());
}